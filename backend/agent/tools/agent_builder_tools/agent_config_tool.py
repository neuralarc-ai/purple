...
class AgentConfigTool(AgentBuilderBaseTool):
    ...
    async def update_agent(
        ...
        try:
            ...
            # Enforce Helium restrictions (simplified)
            if is_helium_default:
                ...
                if restricted_fields:
                    return self.fail_response(
                                f"Cannot modify {', '.join(restricted_fields)} for Helium. "
        f"Helium's core identity is centrally managed. You can still add MCPs, workflows, and triggers."
                    )
