...
function PricingTier({
  ...
  const getPriceId = () => {
    if (billingPeriod === 'yearly' && tier.yearlyStripePriceId) {
      // Legacy yearly plans (hidden from UI but still accessible)
  ...
  const handleSubscribe = async (planStripePriceId: string) => {
    ...
    try {
      ...
      const commitmentType = billingPeriod === 'yearly' ? 'yearly' : 
                          'monthly';
    ...
    if (isCurrentActivePlan) {
      ...
      if (
        ...
        if (targetAmount > currentAmount || isSameTierUpgradeToLongerTerm) {
          ...
          } else if (currentIsYearly && targetIsMonthly && currentTier?.name === tier.name) {
            buttonText = 'Switch to Legacy Yearly';
            buttonVariant = 'default';
            buttonClassName = 'bg-green-600 hover:bg-green-700 text-white';
          } else {
        ...
        {showTitleAndTabs && (
          <SectionHeader>
            ...
            <p className="text-muted-foreground text-center text-balance font-medium">
              Start with our free plan or upgrade for more credits
            </p>
          </SectionHeader>
        )}
