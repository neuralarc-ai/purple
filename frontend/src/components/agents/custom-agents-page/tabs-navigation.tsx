'use client';

import React from 'react';
import { Plus, PackageSearch, LucideProps } from 'lucide-react';
import { FancyTabs, TabConfig } from '@/components/ui/fancy-tabs';
import { Button } from '@/components/ui/button';

// Custom Bot Icon component for My Agents tab - compatible with LucideIcon
const BotIcon = React.forwardRef<SVGSVGElement, LucideProps>(
  ({ className, ...props }, ref) => (
    <svg
      ref={ref}
      width="20"
      height="20"
      viewBox="0 0 24 24"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      {...props}
    >
      <path d="M11 1V2H7C5.34315 2 4 3.34315 4 5V8C4 10.7614 6.23858 13 9 13H15C17.7614 13 20 10.7614 20 8V5C20 3.34315 18.6569 2 17 2H13V1H11ZM6 5C6 4.44772 6.44772 4 7 4H17C17.5523 4 18 4.44772 18 5V8C18 9.65685 16.6569 11 15 11H9C7.34315 11 6 9.65685 6 8V5ZM9.5 9C10.3284 9 11 8.32843 11 7.5C11 6.67157 10.3284 6 9.5 6C8.67157 6 8 6.67157 8 7.5C8 8.32843 8.67157 9 9.5 9ZM14.5 9C15.3284 9 16 8.32843 16 7.5C16 6.67157 15.3284 6 14.5 6C13.6716 6 13 6.67157 13 7.5C13 8.32843 13.6716 9 14.5 9ZM6 22C6 18.6863 8.68629 16 12 16C15.3137 16 18 18.6863 18 22H20C20 17.5817 16.4183 14 12 14C7.58172 14 4 17.5817 4 22H6Z"></path>
    </svg>
  )
);

BotIcon.displayName = 'BotIcon';

interface TabsNavigationProps {
  activeTab: string;
  onTabChange: (value: string) => void;
  onCreateAgent?: () => void;
}

const agentTabs: TabConfig[] = [
  {
    value: 'explore',
    icon: PackageSearch,
    label: 'Explore',
    shortLabel: 'Explore',
  },
  {
    value: 'my-agents',
    icon: BotIcon,
    label: 'My Agents',
  },
];

export const TabsNavigation = ({ activeTab, onTabChange, onCreateAgent }: TabsNavigationProps) => {
  return (
    <div className="flex items-center justify-between w-full">
      <div className="flex-1">
        <FancyTabs
          tabs={agentTabs}
          activeTab={activeTab}
          onTabChange={onTabChange}
          className="max-w-xs"
        />
      </div>

      <div className="flex-shrink-0">
        <Button
          onClick={onCreateAgent}
          className="flex items-center gap-1 rounded-4xl px-4 h-12 font-bold"
        >
          <Plus className="h-4 w-4" />
          Create
        </Button>
      </div>
    </div>
  );
};