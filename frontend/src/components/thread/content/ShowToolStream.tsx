...
export const ShowToolStream: React.FC<ShowToolStreamProps> = ({
  ...
  const toolName: string = getUserFriendlyToolName(rawToolName || '');
  ...
  const streamingFileContent = React.useMemo(() => {
    ...
    if (paramName) {
      const newMatch = content.match(
        new RegExp(`<parameter\\s+name=["']${paramName}["']>([\\s\\S]*)`, 'i'),
      );
        ...
        {shouldShowContent ? (
          ...
          >
            ...
            <div className="relative border-t border-neutral-200 dark:border-neutral-700/50">
              ...
              {/* Top gradient */}
              <div
                className={`absolute top-0 left-0 right-0 h-8 pointer-events-none transition-all duration-500 ease-in-out ${
                  shouldShowContent
              ...
              {/* Bottom gradient */}
              <div
                className={`absolute bottom-0 left-0 right-0 h-8 pointer-events-none transition-all duration-500 ease-in-out ${
                  shouldShowContent
