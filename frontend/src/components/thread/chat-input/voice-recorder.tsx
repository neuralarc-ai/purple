...
export const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
    ...
    const getButtonClass = () => {
        if (isListening) {
            return 'bg-helium-orange text-white hover:bg-helium-orange/90';
        }
        return '';
    };
    ...
    const getIcon = () => {
        if (isListening) {
            return <StopIcon className="mb-0" />;
        }
            ...
            <Tooltip>
                <TooltipTrigger asChild>
                    <Button
                        ...
                        onClick={handleClick}
                        disabled={disabled}
                        className={`h-8 w-8 shadow-none object-contain p-0 bg-transparent border dark:border-muted-foreground/30 rounded-full text-muted-foreground hover:text-foreground hover:bg-background/50! flex items-center justify-center transition-all ${getButtonClass()}`}
                    >
                        {getIcon()}
                    </Button>
                </TooltipTrigger>
